...
const MultiCloudCore = require('fortiweb-autoscale-core');
...
/**
 * Get the public Ip of the given virtual machine
 * @param {String} resourceGroup resource group id
 * @param {String} scaleSetName scale set name
...
async function getVirtualMachinePublicIp(resourceGroup, scaleSetName, instanceId) {
    let resourceId = `/subscriptions/${subscription}/resourceGroups/${resourceGroup}/` +
            ...
            'publicipaddresses';
    let subPubIPId = `/subscriptions/${subscription}/resourceGroups/${resourceGroup}/` +
            `providers/Microsoft.Compute/virtualMachineScaleSets/${scaleSetName}/` +
            `virtualMachines/${instanceId}/networkInterfaces/autoscalenic1/` +
    ...
    for (let pubIP of pubIPs.value) {
        let pubIPId = pubIP.id;
        if (pubIPId.indexOf(subPubIPId) >= 0) {
            publicIp = pubIP.properties.ipAddress;
        }
    }

    logger.info(`called getVirtualMachinePublicIp, publicIp= ${publicIp}.`);
    return publicIp;
}
...
exports.Compute = {
    VirtualMachineScaleSets: {
        ...
        getVirtualMachinePublicIp: getVirtualMachinePublicIp
    }
};
